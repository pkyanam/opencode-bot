...
if (!/^docker\.io\/[a-z0-9._-]+\/[a-z0-9._-]+@sha256:[0-9a-f]{64}$/.test(args.get("--reference"))) throw new Error("reference must be an immutable Docker Hub digest reference");
const output = args.get("--output");
const manifest = {
  schemaVersion: 2, version, commit, opencodeVersion: "2.0.11", sandboxVersion: "0.12.9",
  platform: "linux/amd64", image: { reference: args.get("--reference") }, buildImageDigest: digest,
};
