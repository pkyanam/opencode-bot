...
export const MCP_TOOLS: readonly Tool[] = [
  ...
  tool("memory_remember", "Create a durable memory owned by a bot, optionally shared across computers.", {botId:string("Author bot ID"),title:string("Title"),content:string("Memory content"),kind:string("fact, preference, decision, lesson, procedure, note"),tags:arrayOfStrings("Tags"),visibility:string("private, shared, workspace"),sharedBotIds:arrayOfStrings("Recipient bot IDs")},["botId","content"],create,a=>bodyCall("/api/memory","POST",optionalBody(a,["botId","title","content","kind","tags","visibility","sharedBotIds"]))),
  tool("memory_update", "Correct or change sharing on a memory. Read its latest revision first.",{id:string("Memory ID"),revision:number("Current revision"),title:string("Title"),content:string("Content"),kind:string("Kind"),tags:arrayOfStrings("Tags"),visibility:string("private, shared, workspace"),sharedBotIds:arrayOfStrings("Recipient bot IDs"),pinned:boolean("Always recall this memory")},["id","revision"],update,a=>bodyCall(idPath("/api/memory",required(a,"id")),"PATCH",optionalBody(a,["revision","title","content","kind","tags","visibility","sharedBotIds","pinned"]))),
