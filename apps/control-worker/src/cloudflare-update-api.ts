...
export class CloudflareUpdateApiClient implements CloudflareUpdateApi {
  ...
  async uploadWorkerVersion(input: { bundle: ReleaseBundle; assetsJwt: string }): Promise<{ versionId: string }> {
    ...
    const liveBindings = sanitizeBindings(settings.bindings);
    const optionalBindings = (input.bundle.worker.optionalBindings ?? []).filter((binding) => !liveBindings.some((live) => live.name === binding.name));
    const metadata = { ...input.bundle.worker.metadata, ...(containerMetadata ? {containers: containerMetadata} : {}), main_module: input.bundle.worker.mainModule, compatibility_date: input.bundle.worker.compatibilityDate, ...(input.bundle.worker.compatibilityFlags ? { compatibility_flags: input.bundle.worker.compatibilityFlags } : {}), bindings: [...liveBindings, ...optionalBindings].map(binding => binding.type === "plain_text" && binding.name === "OPENCODE_VERSION" ? { ...binding, text: input.bundle.runtime.opencodeVersion } : binding.type === "plain_text" && binding.name === "SANDBOX_PACKAGE_VERSION" ? { ...binding, text: input.bundle.runtime.sandboxVersion } : binding), keep_bindings: ["secret_text"], assets: { jwt: input.assetsJwt, ...(assetConfig && typeof assetConfig === "object" ? { config: sanitizeAssetConfig(assetConfig as Record<string, unknown>) } : {}) } };
