...
export function ExtensionDiscovery({ botName, onReview }: { botName?: string; onReview: (prompt: string) => void }) {
  ...
  const allRepos = useMemo(() => [...suggestedRepositories.filter(suggested => !repos.some(repo => repo.url === suggested.url)), ...repos], [repos]);
  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="extension-discovery"><DialogTitle>Discover extensions</DialogTitle><DialogDescription>Browse skills and manage native OpenCode plugins.</DialogDescription>
    ...
    <Dialog open={Boolean(selected)} onOpenChange={(value) => !value && setSelected(undefined)}><DialogContent className="extension-detail"><DialogTitle>{selected?.name}</DialogTitle><DialogDescription>{selected?.repository?.url} · {selected?.path}</DialogDescription><div className="extension-file-list"><strong>Files</strong>{(selected?.files || [selected?.path || "SKILL.md"]).map((file) => <code key={typeof file === "string" ? file : file.path}>{typeof file === "string" ? file : file.path}</code>)}</div><pre className="extension-skill-text">{selected?.text || "No skill text was returned."}</pre>{selected && <div className="extension-skill-actions"><Button disabled={Boolean(installing)} onClick={() => void install(selected)}>Install skill</Button>{botName && <Button variant="outline" onClick={() => {onReview(repositoryReviewPrompt(selected.repository!.url));setSelected(undefined);setOpen(false);}}>Ask {botName} to review</Button>}</div>}</DialogContent></Dialog>
  </>;
}
