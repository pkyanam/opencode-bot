...
import { storageDescription } from "../hosting-ui";
...
export function StorageSettings() {
  ...
  const supportsAutomaticCheckpoints = data?.storage?.supportsAutomaticCheckpoints ?? true;
  ...
  return <section className="storage-settings" aria-label="Storage">
    <div className="settings-section-head"><div><h3>Storage</h3><p>{storageDescription(data)}</p></div><button className="icon-btn" aria-label="Refresh storage usage" onClick={() => void load()} disabled={loading}><RefreshCw size={15} className={loading ? "spin" : ""} /></button></div>
    {data?.policyDescription && <p className="settings-muted">{data.policyDescription}</p>}
