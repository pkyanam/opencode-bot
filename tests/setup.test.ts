...
describe("setup planner", () => {
  ...
  it("runs a resumable apply with fake local/cloud commands", () => {
    ...
    writeFileSync(resolve(fixture, ".opencode-bot/computer-image.tar.gz"), archive);
    const commit = execFileSync("git", ["rev-parse", "HEAD"], { cwd: root, encoding: "utf8" }).trim();
    writeFileSync(resolve(fixture, ".opencode-bot/release-manifest.json"), JSON.stringify({ schemaVersion: 1, version: "v0.1.1", commit, platform: "linux/amd64", opencodeVersion: "2.0.11", sandboxVersion: "0.12.9", imageArchive: { file: "computer-image.tar.gz", sha256: createHash("sha256").update(archive).digest("hex"), size: archive.length } }));
    ...
    expect(generated.containers[0].image).toMatch(/^registry\.cloudflare\.com\/[^@]+@sha256:[0-9a-f]{64}$/);
    expect(generated.containers[0].image).not.toMatch(/:v0\.1\.1-/);
    expect(existsSync(resolve(fixture, `.opencode-bot/docker-config-${process.pid}`))).toBe(false);
  });
});
