...
| MCP | `mcp.list`, `mcp.add`, `mcp.remove`, `mcp.connect`, `mcp.disconnect`, `mcp.resource.catalog` | The runtime configures the bundled Playwright MCP server and reports catalog metadata. MCP status, add/remove/connect/disconnect, and resource browsing are a top Web gap; Native `opencode mcp` and the server API remain available. |
...
| `opencode mcp`, `opencode plugin`, `opencode auth`, `opencode api` | manage integrations, plugins, credentials, or call server operations | Native CLI/API; MCP and plugin admin panels remain absent from the web app, while provider credential setup has a web path |
