...
The registry remains the authority for identity, grants, revisions, correction, and deletion. Its Cloudflare Durable Object is authoritative and fail-closed: a provider or projection must never widen a bot's access. Before a task, the coordinator retrieves relevant memories and pinned context within a 6,000-character content budget. Search previews are bounded; `memory_read` retrieves the complete selected record.
...
Each bot has an ACL-scoped Hindsight bank. Shared memories are exposed through an authorized projection, while derived observations and mental models remain scoped to the requesting bot. Corrections and revocations are propagated to provider banks and projections; bank rotation is used when required, and failures fail closed. A disposable Postgres projection cache may accelerate retrieval, but it is rebuilt from the Durable Object registry after a restart. This can add latency and inference cost, and cache contents never become the source of truth.
